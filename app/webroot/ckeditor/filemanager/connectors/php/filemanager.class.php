...
################### add file to media DB ################################
		...
		//	error_log('bootstrap:'.CORE_PATH . 'Cake' . DS . 'bootstrap.php');
    	...
    	$file_path_encoded ="{$this->post['currentpath']}{$_FILES['newfile']['name']}";
    	$file_path_encoded = str_replace(DS, '|sep|', $file_path_encoded);
    	$file_path_encoded = str_replace('|sep|app|sep|webroot|sep|media|sep|', '', $file_path_encoded);
    	$file_path_encoded = urldecode($file_path_encoded);
    	$regeister_path =  "/media/register/$file_path_encoded";
    	
    	$response = array(
			'Path'=>$this->post['currentpath'],
			'Name'=>$_FILES['newfile']['name'],
			'Error'=>"$regeister_path",
			'Code'=>0
		);
		echo '<textarea>' . json_encode($response) . '</textarea>';
	

        $Dispatcher->dispatch(new CakeRequest($regeister_path), new CakeResponse(array('charset' => Configure::read('App.encoding'))));
