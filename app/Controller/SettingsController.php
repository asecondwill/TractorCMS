...
class SettingsController extends AppController {
	...
	function admin_index(){
		...
		if (!empty($this->request->data)) {
			if(!$this->request->data['Setting']['id']) $this->Setting->create();
			
			if ($this->Setting->save($this->request->data)) {
				$this->Session->setFlash(__('The Settings have been saved.'),	'default', array('class'=>'success'));
				

				if (is_uploaded_file($this->request->data['Setting']['favicon']['tmp_name'])){
					$file = new File($this->request->data['Setting']['favicon']['name']);
					
					$ext = $file->ext(); 
					$file->close();
					$file = new File($this->request->data['Setting']['favicon']['tmp_name']);
		...
		if (empty($this->request->data)) {
			$this->request->data = $this->Setting->find();
		}
	}
	...
	function admin_add() {
		if (!empty($this->request->data)) {
			...
			if ($this->SiteText->save($this->request->data)) {
	...
	function admin_edit($id = null) {
		...
		if (empty($this->request->data)) {
			$this->request->data = $this->SiteText->read(null, $id);
		}
	}
