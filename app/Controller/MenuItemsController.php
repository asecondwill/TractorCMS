...
class MenuItemsController extends AppController {
	...
	function admin_order(){
		...
		Configure::write('debug', 0);
           ...
           if($this->RequestHandler->isAjax()) {
             ...
             //  $this->Session->write('BeersInOurSession', $this->request->params['form']['id']);
           }
	}
	...
	function admin_index($menu , $item_id = null) {
		...
		if (empty($this->request->data)) {
			$this->request->data = $this->Menu->read(null, $menu);
		}
		$this->set('menu', $this->request->data);
	}
	...
	//debug($menu_id);
		...
		$lists = $this->MenuItem->List->find('list');
		$this->set(compact('lists'));
		$this->set('menuItem', $this->request->data);
		$this->set('parent_id', $parent_id);
		$this->set('menu_id', $menu_id);
	}
