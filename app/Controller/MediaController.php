...
class MediaController extends AppController {
	...
	function beforeFilter() {			
	 	if($this->request->action=='upload'){	 		 		
		    CakeSession::id($this->request->params['pass'][0]);
            CakeSession::start();          
       	}               
        parent::beforeFilter();                
    } 
	...
	function admin_listing(){
	    ...
	    if (!empty($this->request->params['width'])) {
	    	$filter['width'] = 	$this->request->params['width'];
	    	$this->set('width', $this->request->params['width']);	
	    }
	    if (!empty($this->passedArgs['width_max'])) {
	    	$filter['width <='] = 	$this->passedArgs['width_max'];
	    	$this->set('width_max', $this->passedArgs['width_max']);	
	    }
	    if (!empty($this->request->params['width_max'])) {
	    	$filter['width <='] = 	$this->request->params['width_max'];
	    	$this->set('width_max', $this->request->params['width_max']);	
	    }
	    ...
	    if (!empty($this->request->params['height_max'])) {
	    	$filter['height <='] = 	$this->request->params['height_max'];
	    	$this->set('height_max', $this->request->params['height_max']);	
	    }	    
	    if (!empty($this->passedArgs['height'])) {
	    	$filter['height'] = 	$this->passedArgs['height'];
	    	$this->set('height', $this->passedArgs['height']);	
	    }
	    if (!empty($this->request->params['height'])) {
	    	$filter['height'] = 	$this->request->params['height'];
	    	$this->set('height', $this->request->params['height']);	
	    }
	...
	function admin_index() {
		...
		if(isset($this->request->params['named']['sort'])) { // user clicked on a "sort by" link, write his choice to the session
		  $this->Session->write('media_sort',array($this->request->params['named']['sort']=>$this->request->params['named']['direction']));
		}
		...
		if(isset($this->request->params['named']['page'])) { // user clicked on a "page" link, write his choice to the session
		  $this->Session->write('media_page',$this->request->params['named']['page']);
		}
	...
	function admin_edit( $id = null) {
		...
		if (!empty($this->request->data)) {
			if ($this->Content->save($this->request->data)) {
				...
				$this->redirect(array('action'=>'index', $this->request->data['Media']['content_type']));
			} else {
